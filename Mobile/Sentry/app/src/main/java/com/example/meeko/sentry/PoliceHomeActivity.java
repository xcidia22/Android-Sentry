...
public class PoliceHomeActivity extends FragmentActivity implements OnMapReadyCallback, LocationListener {
    ...
    Runnable checkstats = new Runnable() {
        ...
        public void run() {
            ...
            Log.v("CHECKING123",""+pref.getBoolean("LoggedIn",true));
    ...
    Runnable getLoc = new Runnable() {
        ...
        public void run() {
            ...
            CollectionReference LocationsRef = db.collection("Officers");
            LocationsRef.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isSuccessful()) {
                        SharedPreferences pref = getApplicationContext().getSharedPreferences("MyPref", MODE_PRIVATE);
                        String Username = pref.getString("userPhone", "");
                        String tempID = "";
                        getDeviceLocation();
                        if (tempID=="") {
                            for (QueryDocumentSnapshot documentSnapshot : task.getResult()) {
                                if (Username.equals(documentSnapshot.get("mobile"))) {
                                    tempID = documentSnapshot.getId();
                                    FirebaseFirestore db = FirebaseFirestore.getInstance();
                                    DocumentReference update = db.collection("Officers").document(tempID);
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        edit.putString("policeLoggedIn","true");
        edit.commit();
        FacebookSdk.sdkInitialize(this);
        mobile=pref.getString("userPhone","");
        Log.v("Hey",mobile);
        Log.v("WORKS","WELL");
        ...
        NavigationView navigationView = (NavigationView) findViewById(R.id.police_nav_view);

        View navHeaderView = navigationView.inflateHeaderView(R.layout.police_header);
        TextView headerName = (TextView) navHeaderView.findViewById(R.id.policeName);
        TextView headerEmail = (TextView) navHeaderView.findViewById(R.id.policeEmail);
        headerName.setText(name);
        headerEmail.setText(email);

        new PoliceHomeActivity.DownloadImage((ImageView)navHeaderView.findViewById(R.id.policeProfileImage)).execute(imageUrl);

        mDrawerLayout = findViewById(R.id.policedrawer);
        navigationView.setNavigationItemSelectedListener(
                new NavigationView.OnNavigationItemSelectedListener() {
                    ...
                    public boolean onNavigationItemSelected(MenuItem menuItem) {
                        ...
                        if (id == R.id.ice) {
                            Intent iceIntent = new Intent(PoliceHomeActivity.this, ICEActivity.class);
                            startActivity(iceIntent);
                        }
                        else if (id == R.id.incidents) {
                            Intent helpIntent = new Intent(PoliceHomeActivity.this, IncidentList.class);
                            startActivity(helpIntent);
                        }
                        else if (id == R.id.setup_pass) {
                            Intent lockScreenIntent = new Intent(PoliceHomeActivity.this, LockScreenActivity.class);
                            ...
                            Intent helpIntent = new Intent(PoliceHomeActivity.this, HelpCentreActivity.class);
                            startActivity(helpIntent);
    ...
    public void onClickLogout(View v){
        ...
        SharedPreferences pref = getApplicationContext().getSharedPreferences("MyPref", MODE_PRIVATE);
        SharedPreferences.Editor edit = pref.edit();
        edit.putString("policeLoggedIn","");
        edit.putBoolean("LoggedIn",false);
        edit.commit();
        login = false;
        Log.v("ErrorLogout",""+pref.getBoolean("LoggedIn",true));
        Intent login = new Intent(PoliceHomeActivity.this, LoginActivity.class);
        startActivity(login);
    ...
    public void onClickHidden(View view) {
        if (isNetworkConnected()) {
            ...
            final CollectionReference LocationsRef = db.collection("Officers");
            LocationsRef.get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                @Override
                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                    if (task.isSuccessful()) {
                        ...
                        for (QueryDocumentSnapshot documentSnapshot : task.getResult()) {
                            if (Username.equals(documentSnapshot.get("mobile"))) {
                                tempID = documentSnapshot.getId();
                                DocumentReference update = db.collection("Officers").document(tempID);
                                if (documentSnapshot.getBoolean("hidden")) {
                ...
                .addSnapshotListener(new EventListener<QuerySnapshot>() {
                    ...
                    public void onEvent(@Nullable QuerySnapshot queryDocumentSnapshots, @Nullable FirebaseFirestoreException e) {
                        ...
                        for (QueryDocumentSnapshot doc : queryDocumentSnapshots) {
                            ...
                            } else if (doc.get("hidden").equals(true)&&PoliceMarker[x]!=null){
                                PoliceMarker[x].remove();
                                PoliceMarker[x]=null;
                            } else {
                                PoliceMarker[x]=null;
                            }
            ...
            public void onInfoWindowClick(Marker marker) {
                if(marker.getTitle().toLowerCase().startsWith("p")){
                    ...
                    try {
                        ...
                        Toast.makeText(PoliceHomeActivity.this,"Please Turn On Call Permissions!", Toast.LENGTH_SHORT);
        ...
        h.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if(ONGOING==true){
                    ...
                    Toast.makeText(PoliceHomeActivity.this, "You have an ongoing incident!", Toast.LENGTH_SHORT).show();
                }
                else{
                ...
                if(txttxt.getText().toString()!=""&temp!=null) {
                    for (int i = 0; i < LocationController.length; i++) {
                        if (mpolt.PointIsInRegion(temp.latitude, temp.longitude, LocationController[i])&LocationValues[i]>6) {
                            showNotification(PoliceHomeActivity.this,"You are in a crimezone!","Caution is advised!",new Intent(PoliceHomeActivity.this,PoliceHomeActivity.class));
                        ...
                        public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                            ...
                            for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                                if (documentSnapshot.get("Ongoing").equals(true)) {
                                    ...
                                    if (dist <= 150) {
                                        nearIncident=true;
                                        showNotificationNearIncident(PoliceHomeActivity.this, "You are near an Incident!", "Nearest Incident: " + (int) dist + "meters", new Intent(PoliceHomeActivity.this, PoliceHomeActivity.class));
        ...
        if (isNetworkConnected()&&LocationProvider&&temp!=null) {
            if(nearIncident&&witnessReport){
                ...
                builder2.setPositiveButton("No", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        AlertDialog.Builder builder1 = new AlertDialog.Builder(PoliceHomeActivity.this);
            ...
            Toast.makeText(PoliceHomeActivity.this, "No Internet Connection!", Toast.LENGTH_SHORT).show();
        }
    }
